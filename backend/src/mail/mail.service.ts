...
export class MailService {
    // Vercel uzerindeki mail gonderme endpoint'i (SMTP bypass)
    private get vercelMailApiUrl(): string {
        const appUrl = process.env.APP_URL || 'https://frontend-nu-ruddy-70.vercel.app';
        return `${appUrl}/api/send-email`;
    }

    constructor() { }

    private async callVercelApi(toEmail: string, subject: string, html: string): Promise<void> {
        try {
            console.log(`Vercel SMTP uzerinden e-posta gonderiliyor: ${toEmail}`);
            const response = await fetch(this.vercelMailApiUrl, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                console.error('Vercel e-posta gonderim hatasi:', result);
                throw new Error('Vercel API hatasi: ' + (result.error || result.message));
            }
            console.log('E-posta basariyla teslim edildi (Vercel API).');

        } catch (error) {
            console.error('HTTP fetch hatasi (Vercel API ulasilamadi):', error);
            throw error;
        }
    }
    ...
    async sendVerificationEmail(toEmail: string, token: string): Promise<void> {
        ...
        const subject = 'Pusula Oto Kiralama - E-posta Adresinizi Doğrulayın';
        const html = `
        ...
        await this.callVercelApi(toEmail, subject, html);
    }
    ...
    async sendPasswordResetEmail(toEmail: string, token: string): Promise<void> {
        ...
        const subject = 'Pusula Oto Kiralama - Şifre Sıfırlama';
        const html = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                <div style="background: linear-gradient(135deg, #1e3a5f, #2563eb); padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
                    <h1 style="color: white; margin: 0; font-size: 24px;">Pusula Oto Kiralama</h1>
